...
abstract class RoutingModule[F[_]: Async: std.Console: Trace: Network] {
  ...
  final protected[modules] def withRoutes(dbConfig: DatabaseConfig): Resource[F, HttpApp[F]] = for {
  ...
  private[this] def initServiceLogic(session: Session[F]): F[List[ServerEndpoint[Any, F]]] = {
    ...
    lazy val serviceLogic: ServiceLogic[F] = new ServiceLogic[F](employeeRepo, addressRepo)
    serviceLogic.make
  }
